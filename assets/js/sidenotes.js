...
function revealTarget() {
    ...
    let target = document.getElementById(decodeURIComponent(location.hash).substr(1));
    if (!target) return;
...
function updateSidenotePositions() {
    ...
    /*  Position left sidenote column so top is flush with top of first
        ...
        */
    let markdownBody = document.querySelector(".container");
...
function constructSidenotes() {
    ...
    for (var i = 0; i < GW.sidenotes.footnoteRefs.length; i++) {
        ...
        let referencedFootnote = document.getElementById(GW.sidenotes.footnoteRefs[i].hash.substring(1));
